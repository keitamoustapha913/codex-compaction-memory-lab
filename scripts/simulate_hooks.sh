...
mkdir -p .codex/memory/transcript-snapshots
cat > .codex/memory/fake-transcript.jsonl <<'EOF'
...
TRANSCRIPT="$ROOT/.codex/memory/fake-transcript.jsonl"
...
if ! grep -q '"event": "PreCompact"' .codex/memory/compaction-events.jsonl; then
  echo "Missing PreCompact marker" >&2
  exit 1
fi

if ! grep -q '"event": "PostCompact"' .codex/memory/compaction-events.jsonl; then
  echo "Missing PostCompact marker" >&2
  exit 1
fi

if ! grep -q 'Simulated prompt' .codex/memory/prompt-events.jsonl; then
  echo "Missing prompt marker" >&2
  exit 1
fi

if ! find .codex/memory/transcript-snapshots -type f -name '*precompact.jsonl' | grep -q .; then
  echo "Missing precompact transcript snapshot" >&2
  exit 1
fi

if ! find .codex/memory/transcript-snapshots -type f -name '*postcompact.jsonl' | grep -q .; then
